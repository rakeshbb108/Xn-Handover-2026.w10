...
void createXninst(instance_t instanceP, xnap_setup_info_t *info, xnap_net_config_t *nc)
{
  ...
  if (info)
    xn_inst[0]->setup_info = *info;
  if (nc)
    xn_inst[0]->net_config = *nc;
  pthread_mutex_unlock(&xn_inst_mtx);
}

void updateXninst(instance_t instanceP, xnap_setup_info_t *info, xnap_net_config_t *nc, sctp_assoc_t assoc_id)
{
  DevAssert(instanceP == 0);
  pthread_mutex_lock(&xn_inst_mtx);
  AssertFatal(xn_inst[instanceP] != NULL, "XN instance not found\n");
  if (info)
    xn_inst[instanceP]->setup_info = *info;
  if (nc)
...
{
  printf("Handling xn setup message, SCTP_SHUTDOWN: %d \n", sctp_shutdown);
  if (sctp_shutdown) {
    ...
    if (gnb_data_p->state == XNAP_GNB_STATE_CONNECTED) {
      ...
      MessageDef *message_p = itti_alloc_new_message(TASK_XNAP, 0, XNAP_LOST_CONNECTION);
      xnap_lost_connection_t *msg  = &XNAP_LOST_CONNECTION(message_p);
      msg->assoc_id = assoc_id;          
      itti_send_msg_to_task(TASK_RRC_GNB, instance, message_p);
      LOG_I(XNAP, "Informing RRC about Lost XNAP connection with assoc_id:%d\n", assoc_id);
